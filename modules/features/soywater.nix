...
in
{
  ...
  { config, lib, pkgs, ... }:
  {
    options.settingsDir = lib.mkOption {
      type = lib.types.str;
      default = "${config.users.users.${username}.home}/nixconfigs/modules/features";
      description = "Directory containing this user's declarative settings.";
    };
